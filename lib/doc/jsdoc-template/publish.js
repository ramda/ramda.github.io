...
var logger = require('jsdoc/util/logger'); // for debugging.  `logger.warn(...)` etc.
...
// handle any combination of comma-separated and multi-line @see annotations
...
function simplifyData(d) {
  ...
  var akaStr = (valueForTitle('aka', d.tags) || '');
  var aka = akaStr && akaStr.split(/,\s*/) || [];
  if (aka.length) {
      logger.warn(d.name + ' aka ' + JSON.stringify(aka));
  }
  return {
    name: d.name || '',
    sig: sig,
    ...
    returns: {
      type: returns && returns.type && returns.type.names ? returns.type.names[0] : '',
...
var pkg = loadJson('tmp/package.json');
var readme = loadMarkdown('tmp/README.md');
